...
resource "volterra_azure_vnet_site" "azure-site2" {
  name        = format("%s-azure-2", var.projectPrefix)
  namespace      = "system"
  azure_region   = var.azureRegion2
  resource_group = "${var.resourceGroup}-site2"
  ...
  ingress_egress_gw {
    ...
    az_nodes {
      ...
      inside_subnet {
        subnet {
          subnet_name         = "internal_subnet2"
          vnet_resource_group = true
        }
      }
      outside_subnet {
        subnet {
          subnet_name         = "external_subnet2"
          vnet_resource_group = true
        }
      }
    }
...
resource "volterra_cloud_site_labels" "labels" {
  name = volterra_azure_vnet_site.azure-site2.name
...
resource "volterra_tf_params_action" "azure-site2" {
  site_name        = volterra_azure_vnet_site.azure-site2.name
  site_kind        = "azure_vnet_site"
  action           = "apply"
  wait_for_action  = true
  ignore_on_update = false

  depends_on = [volterra_azure_vnet_site.azure-site2]
}
